...
def submit_answer(level_source, question_nr, attempt):
    ...
    if is_correct:
        ...
        quiz_answers = DATABASE.get_quiz_answer(username, level_source, session['quiz-attempt-id'])
        return redirect(
            url_for('quiz_feedback', quiz_answers=quiz_answers, level_source=level_source, question_nr=question_nr,
                    lang=g.lang))
    ...
    quiz_answers = DATABASE.get_quiz_answer(username, level_source, session['quiz-attempt-id'])
    return redirect(
        url_for('get_quiz', quiz_answers=quiz_answers, chosen_option=chosen_option, level_source=level_source,
                question_nr=question_nr,
...
def quiz_feedback(level_source, question_nr):
    ...
    quiz_answers = DATABASE.get_quiz_answer(username, level_source, session['quiz-attempt-id'])
    return render_template('feedback.html', quiz_answers=quiz_answers, question=question,
                           questions=questions,
